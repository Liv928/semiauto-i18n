  ...
  <div class="import-resume">
    <div class="header">
      <div class="item">
        ...
        <span class="process-text">导入候选人简历</span>
      </div>
      <div class="item">
        <div class="line" />
      </div>
      <div class="item">
        ...
        <span class="process-text">简历解析预览</span>
      </div>
    </div>
        ...
        <div slot="upload">
          <div v-loading="loading" class="upload-container">
            ...
            >
              ...
              <div class="el-upload__text"><em>点击上传多份简历</em></div>
              <div class="el-upload__text upload-prompt">（可上传多份简历，支持pdf、doc、docx格式，大小不超过10M）</div>
            </el-upload>
          </div>
        </div>
      </bx-filter>
      <div class="text">没有简历或简历解析不成功？只需录入简单的信息就可以添加候选人</div>
      <div class="text"> <span class="click" @click="addApplicant">手动添加候选人</span></div>
    </div>
        ...
        <div class="content-right">
          ...
          <div class="content-container" style="margin:10px">
            <div class="content-tl">
              <div class="title">
                <span class="vertical-line" />
                <span class="title-text">入库信息</span>
              </div>
            </div>
            <bx-filter
              ref="filter_three"
    ...
    <div class="btn-group">
      <el-button v-if="importSteps=='1'" style="float:right;margin-right: 20px" type="primary" @click="submit">提交</el-button>
      <el-button v-if="importSteps=='2'" style="float:right;margin-right: 20px" type="primary" @click="saveInfo">保存</el-button>
...
export default {
      ...
      uploadConfig: [
        {
          ...
          props: {
            ...
            required: true,
            validator: { text: '不能为空', reg: 'required' }
          }
        },
        {
          type: 'select',
          ...
          props: {
            ...
            data: {
              type: 'data',
              options: [
                { label: '初筛', value: 'AB001' },
                { label: '简历评估', value: 'AB002' },
                { label: '面试', value: 'AB003' },
                { label: '录用', value: 'AB004' }
              ]
            },
            required: true,
            validator: { text: '不能为空', reg: 'required' }
  ...
  methods: {
    ...
    getUserInfoForm(positionId) {
      ...
      this.$http({
        ...
        if (res && res.isSuccess) {
          ...
          // console.log(a, configData.filter(i => i !== undefined || i !== null || i !== 'undefined'), '动态获取得数据')
        }
      }).catch(() => {
        this.formLoading = false
      })
    ...
    resumeDataSelect(index) {
      console.log(this.fileList[index], '数据')
        ...
        (data) => {
          if (data === 1) {
            ...
            this.$http
              ...
              .catch(() => {
                this.$message.error('保存候选人失败')
              })
          } else if (data === 3) {
            if (this.resumeDataIndex + 1 === this.fileList.length) {
              this.$message.warning('已经是最后一条简历')
